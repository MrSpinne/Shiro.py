...
class Settings(commands.Cog):
	...
	async def prefix(self, ctx, prefix: converters.LengthStr(1, 10)):
		...
		embed.description = _("Server prefix were set to `{0}`. If you forget it, you can always use `@Shiro` to get help.")
	...
	async def channel(self, ctx, channel: typing.Union[converters.Nothing, discord.TextChannel]):
		...
		embed.description = _("Command can now be executed {0}.").format(f"everywhere" if channel is None else f"in {channel.mention}")
		await ctx.send(embed=embed)
